...
export default function Box({
  ...
  const stopClickPropagation = (e) => {
    e.stopPropagation(); // Empêche la propagation du clic vers les parents
  };
  return (
    <div
    ...
    >
      <div className="w-[20%] flex justify-center max-2xl:mt-[1vw] items-center relative">
        <div className="w-full flex justify-center items-center relative">
          <FirstIcone
            color={color}
            className={`w-[3vw] h-[2.5vw] max-sm:invisible`}
          />
          <SecondIcone
            color={color}
            className={`max-sm:w-[32px] max-sm:h-[32px] w-[3vw] h-[2.5vw] absolute z-10 right-0 bottom-0 transform max-sm:relative translate-x-[20%] translate-y-[20%] opacity-100`}
          />
