...
import { selectPage, selectStation } from "@/features/Stations/stationSelector.js";


const datas = ["id", "adresse", "status", "charge_point_model", "charge_point_vendors", "Actions"];
const columns = Columns(datas);
const actions = [{ name: "detail" }, { name: "edit" }, { name: "delete" }]
const DataTableStation = () => {
    const currentPage = useSelector(selectPage)
    const { isPending, error, data } = StationApi('cp/read_cp', 'repoStation', currentPage, 10)
